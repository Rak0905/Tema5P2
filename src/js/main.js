...
function showCategories() {
  ...
  restaurantsManager.categories.forEach(category => {
    ...
    categoryLink.addEventListener('click', () => showCategoryDetails(category));
...
function showAllergensWithDishes() {
  ...
  restaurantsManager.allergens.forEach(allergen => {
    ...
    if (dishesWithAllergen.length > 0) {
      ...
      // Agregar evento de clic para toggle la visibilidad de los platos
      allergenItem.addEventListener('click', () => {
        dishesList.style.display = dishesList.style.display === 'none' ? 'block' : 'none';
      });
    } else {
